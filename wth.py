...
def identify_hash(hash_string):
    ...
    if not re.match(r"^[a-fA-F0-9]+$", hash_string):
        print(f"[ERROR] Invalid hash: {hash_string}. Hashes should contain only hex characters.")
        return None
    ...
    for hash_type, signature in hash_signatures.items():
        if len(hash_string) == signature["length"] and re.match(signature["characters"], hash_string):
            ...
            print(f"[INFO] Exact match found: {hash_type}")
            return hash_type
    ...
    if confidence >= 0.75:  # Show closest match only if confidence is high
        print(f"[WARNING] Closest match: {closest_type} (Confidence: {confidence:.2f})")
        log_identification_attempt(hash_string, f"{closest_type} (Confidence: {confidence:.2f})")
        return closest_type
    else:
        print(f"[ERROR] Unknown hash type for: {hash_string}")
        return "Unknown"
...
def compare_hashes(hash1, hash2):
    if hash1 == hash2:
        print("[INFO] The hashes are identical.")
    else:
        print("[INFO] The hashes are different.")
...
def identify_and_lookup(hash_string):
    ...
    if identified_type:
        ...
        if identified_type in ["md5", "sha1"]:
            ...
            if original_value:
                print(f"[INFO] Original value found for {identified_type}: {original_value}")
            else:
                print(f"[INFO] No known plaintext value found for {identified_type}")
...
def main():
    ...
    if args.hash:
        ...
        print("[ERROR] Please provide a hash string with -s, a file with -f, or hashes to compare with -c.")
        parser.print_help()
